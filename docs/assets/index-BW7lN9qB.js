(function(){const l=document.createElement("link").relList;if(l&&l.supports&&l.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))u(a);new MutationObserver(a=>{for(const n of a)if(n.type==="childList")for(const c of n.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&u(c)}).observe(document,{childList:!0,subtree:!0});function s(a){const n={};return a.integrity&&(n.integrity=a.integrity),a.referrerPolicy&&(n.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?n.credentials="include":a.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function u(a){if(a.ep)return;a.ep=!0;const n=s(a);fetch(a.href,n)}})();const K="album-db",b="photos",Q=1;function B(){return new Promise((t,l)=>{const s=indexedDB.open(K,Q);s.onupgradeneeded=u=>{const a=u.target.result;a.objectStoreNames.contains(b)||a.createObjectStore(b,{keyPath:"id",autoIncrement:!0})},s.onsuccess=()=>t(s.result),s.onerror=()=>l(s.error)})}async function W(t){const l=await B();return new Promise((s,u)=>{const c=l.transaction(b,"readwrite").objectStore(b).add({...t,createdAt:new Date().toISOString()});c.onsuccess=()=>s(c.result),c.onerror=()=>u(c.error)})}async function C(){const t=await B();return new Promise((l,s)=>{const n=t.transaction(b,"readonly").objectStore(b).getAll();n.onsuccess=()=>l(n.result),n.onerror=()=>s(n.error)})}async function X(){const t=await B();return new Promise((l,s)=>{const n=t.transaction(b,"readwrite").objectStore(b).clear();n.onsuccess=()=>l(),n.onerror=()=>s(n.error)})}async function Y(t){const l=await B();return new Promise((s,u)=>{const a=l.transaction(b,"readwrite"),n=a.objectStore(b);let c=0;t.forEach(g=>{const m=n.add({...g,importedAt:new Date().toISOString()});m.onsuccess=()=>{c++,c===t.length&&s()},m.onerror=x=>{console.error("Erro ao adicionar item",x)}}),a.oncomplete=()=>{},a.onerror=()=>u(a.error)})}async function Z(t,l=600){return new Promise((s,u)=>{const a=new Image;a.onload=()=>{const n=a.width/a.height,c=Math.min(l,a.width),g=Math.round(c/n),m=document.createElement("canvas");m.width=c,m.height=g;const x=m.getContext("2d");x.fillStyle="#fff",x.fillRect(0,0,c,g),x.drawImage(a,0,0,c,g);const M=m.toDataURL("image/jpeg",.85);s(M)},a.onerror=n=>u(n),a.src=t})}const k=8;function ee(){const t=document.createElement("div");t.className="min-h-screen py-8 px-4 bg-amber-50 flex justify-center",t.innerHTML=`
  ...
  `;const l=t.querySelector("#fileInput"),s=t.querySelector("#uploadBtn"),u=t.querySelector("#clearBtn"),a=t.querySelector("#exportBtn"),n=t.querySelector("#importFile"),c=t.querySelector("#toggleNotes"),g=t.querySelector("#gallery-front"),m=t.querySelector("#gallery-back"),x=t.querySelector(".album-frame");t.querySelector(".flip-container");const M=t.querySelector("#prevBtn"),U=t.querySelector("#nextBtn"),O=t.querySelector("#pageIndicator"),w=t.querySelector("#modalRoot"),$=t.querySelector("#title"),j=t.querySelector("#date"),A=t.querySelector("#tags"),T=t.querySelector("#desc"),N=t.querySelector("#special");let L=[],v=0,h=1,I=!0,q=!0;async function S(){L=await C(),L.sort((r,o)=>(o.date||"").localeCompare(r.date||"")),h=Math.max(1,Math.ceil(L.length/k)),v=Math.min(v,h-1),O.textContent=`${v+1} / ${h}`;const e=R(v);q?(g.innerHTML="",g.append(...e)):(m.innerHTML="",m.append(...e))}function R(e){const r=[],o=e*k,d=L.slice(o,o+k);d.forEach(i=>{var F;const f=document.createElement("article"),E=Math.random()*8-4;f.className="polaroid",f.style.setProperty("--angle",`${E}deg`);const P=I&&i.description?`<div class="sticky-note">${p(V(i.description,90))}</div>`:"",D=i.special?'<div class="ribbon">Especial</div>':'<div class="ribbon">Memória</div>';f.innerHTML=`
        ...
        ${P}
      `,f.querySelector(".photo-link").addEventListener("click",z=>{z.preventDefault(),_(i)}),r.push(f)});const y=k-d.length;for(let i=0;i<y;i++){const f=document.createElement("div");f.className="",r.push(f)}return r}async function H(e){if(e<0||e>=h||e===v)return;const r=e;q=!q;const o=q?g:m;o.innerHTML="",o.append(...R(r)),x.classList.add("is-flipping"),await G(750),v=r,O.textContent=`${v+1} / ${h}`,x.classList.remove("is-flipping")}function G(e){return new Promise(r=>setTimeout(r,e))}function _(e){var r;w.innerHTML=`
